...
#property description "XAUUSD COUNTER TREND v2.24 - Balanced-frequency corrected retracement"
#property strict
...
input double   InpPDThreshold          = 0.350;                 // Retracement depth from the swing extreme (0.350 = 35%)
...
void InitStateFromHistory()
{
   ...
   for(int i = hb-2*InpPivotLength-1; i >= 2; i--)
   {
      ...
      // v2.23+: InpPDThreshold is a retracement depth from the impulse
      // extreme. For a bullish swing, 0.35 means price must retrace 35%
      // down from swing_high; for a bearish swing, it must retrace 35%
      // up from swing_low. v2.24 uses the validated balanced-frequency depth.
      double dl=swing_high-(sr*InpPDThreshold), pl2=swing_low+(sr*InpPDThreshold);
...
void ExecuteStrategyLogic()
{
   ...
   // 5. Premium / Discount
   double sr=swing_high-swing_low;
   // Same corrected retracement contract as InitStateFromHistory():
   // BUY discount is measured down from the high, SELL premium up from low.
   double dl=swing_high-(sr*InpPDThreshold), pl2=swing_low+(sr*InpPDThreshold);
   ...
   if(InpUseEMA)
   {
      ...
      if(!ema_ready)
      {
         // A handle created once in OnInit() can stay permanently stuck in
         // Strategy Tester. Recreate it instead of blocking all entries for
