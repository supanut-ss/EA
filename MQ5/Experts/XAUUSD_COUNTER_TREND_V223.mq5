...
#property description "XAUUSD COUNTER TREND v2.23 - Corrected premium/discount retracement depth"
#property strict
...
input double   InpPDThreshold          = 0.550;                 // Retracement depth from the swing extreme (0.550 = 55%)
...
void InitStateFromHistory()
{
   ...
   for(int i = hb-2*InpPivotLength-1; i >= 2; i--)
   {
      ...
      // extreme. For a bullish swing, 0.55 means price must retrace 55%
      // down from swing_high; for a bearish swing, it must retrace 55%
      // up from swing_low. v2.22 used the complementary 30% level here.
